...
void main(){
	...
	//the offset is only applied in the x direction
	vec2 rpx = vertTexCoord.st + vec2(texOffset.s * roff, 0);
	vec2 gpx = vertTexCoord.st + vec2(texOffset.s * goff, 0);
	vec2 bpx = vertTexCoord.st + vec2(texOffset.s * boff, 0);
	//grab the colour at the texel for each offset
	vec4 colr = texture2D(texture, rpx);	
	vec4 colg = texture2D(texture, gpx);	
	vec4 colb = texture2D(texture, bpx);	
  	
  	vec2 pos = gl_FragCoord.xy;
  	//vec4 sum = 8.0 * col4 - (col0 + col1 + col2 + col3 + col5 + col6 + col7 + col8); 
  	gl_FragColor = vec4(colr.r, colg.g, colb.b, 1.0) * vertColor;
}